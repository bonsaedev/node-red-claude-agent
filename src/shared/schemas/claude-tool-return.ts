...
/**
 * `claude-tool-return` config. Minimal: the resolver is recovered off the private
 * channel; only the output FORMAT is a config choice. `isError` is driven per
 * message (`msg.output.isError`), not config.
 */
